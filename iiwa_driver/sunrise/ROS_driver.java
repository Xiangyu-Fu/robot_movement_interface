...
public class ROS_driver extends RoboticsAPIApplication {
	...
	public void initialize() {
		...
		tool = createFromTemplate("GripperWSG50");
	...
	String process(String command, String[] parameters){
		...
		if (command.compareToIgnoreCase("smart joint move") == 0){
			...
			try {
				...
				double blending = Double.parseDouble(parameters[14]);
				...
				motionContainer = tool.moveAsync(ptp(jointPosition).setJointVelocityRel(relativeSpeed.get()).setBlendingRel(blending));		
